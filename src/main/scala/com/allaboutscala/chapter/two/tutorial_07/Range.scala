...
object Range extends App {
  ...
  println("\nStep 2: Create a numeric range from 1 to 5 but exclusding the integer number 5")
  val from1Until5 = 1 until 5
  println(s"Range from 1 until 5 where 5 is excluded = $from1Until5")

  println("\nStep 3: Create a numeric range from 0 to 10 by increment with multiples of 2")
  ...
  println("\nStep 5:Character range with user specified increment")
  val alphabetRangeFromAToZ2 = 'a' to 'z' by 2
  println(s"Range of every other alphabet = $alphabetRangeFromAToZ2")
